...
long ICC_Reader_Application(JNIEnv *env,unsigned char slot,long command_len,unsigned char *apdu,unsigned char *respond_apdu);
long ICC_Reader_Libinfo(JNIEnv *env,char *info);
long ICC_Reader_GetDevID(JNIEnv *env, char *device_id);
long Activation_Rfcard(JNIEnv *env,long time_out,unsigned char *card_type,long *uid_len,unsigned char *card_uid,long *atr_len,unsigned char *atr);
long Application_Rfcard(JNIEnv *env,long apdu_len,unsigned char *apdu,long *respond_len,unsigned char *respond_apdu);
...
long Get_Magnetic_Card(JNIEnv *env,unsigned char *data);
long Get_HardWare_Version(JNIEnv *env, char *info);
long Lib_Version(char *Info);
long Test_Beep(JNIEnv *env);
long Reader_UpgraderMode(JNIEnv *env);
long Syn_Set_Configuration(JNIEnv *env);
void error_log(int code,int dev_type,const char * emsg);
void record_f4mds_msg(int code);
}
